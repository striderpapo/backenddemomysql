...
console.log(producto)
  ...
  pool.query(sql, function (err, productoStored) {
    if (err) throw err;
...
pool.query(sqlSelect, [productoId, iduser], function (err, results) {
    if (err) throw err;
    if (!results.length) {
...
console.log(publicId)
  ...
  cloudinary.uploader.destroy(publicId, function (error, result) {
        ...
        pool.query(sqlDelete, [productoId, iduser], function (err, deleteResult) {
            if (err) throw err;
	...
	updateProduct:function(req,res){
	  ...
	  pool.query(sql, function (err, pruebaStored) {
    if (err) throw err;
...
pool.query(sqlSelect, [idagrego], function (err, results) {
    if (err) throw err;
    if (!results.length) {
...
console.log(results[0].iduser+"-"+results[0].user)
  ...
  pool.query(sql, [result.url,productoId,idagrego], function (err, pruebaStored) {
    if (err) throw err;
    console.log("1 record update");
    console.log(pruebaStored);
    return res.status(200).send({prueba:pruebaStored});
